...
export const apiSwap = async () => {
  /**
   * 
   * Please note that the refer function is currently only available for input mints of SOL, WSOL, USDC, and USDT. Other parameters will be ignored by default without triggering an error.
   * 
   */
  ...
  const referrerBps = 100
  const referrerWallet = '<referrer wallet address>'
  ...
  const { data: swapResponse } = await axios.get<SwapCompute>(
    ...
    }&txVersion=${txVersion}&referrerBps=${referrerBps}`
  )
